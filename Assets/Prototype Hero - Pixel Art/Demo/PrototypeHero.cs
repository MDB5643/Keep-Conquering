...
public class PrototypeHero : MonoBehaviour {
    ...
    void Update ()
    {
        ...
        if (m_timeSinceStun > 4.0f)
        {
            ...
            // Throw
            else if ((Input.GetKey("d") || Input.GetKey("a")) && Input.GetMouseButtonDown(1) && !m_dodging && !m_ledgeGrab && !m_ledgeClimb && m_timeSinceAttack > 0.3f && m_timeSinceSideSpecial > 2.0f && !m_animator.GetBool("isParrying"))
            {
