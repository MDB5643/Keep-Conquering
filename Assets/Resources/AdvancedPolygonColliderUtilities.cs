...
namespace DigitalRuby.AdvancedPolygonCollider
{
    public sealed class TextureConverter
    {
        ...
        public List<Vertices> DetectVertices(UnityEngine.Color[] colors, int width, int alphaTolerance, float cropXMax, float cropYMax, float cropXMin, float cropYMin)
        {
            this.width = width;
            ...
            List<Vertices> detectedVerticesList = DetectVertices(cropXMax, cropYMax, cropXMin, cropYMin);
        ...
        public List<Vertices> DetectVertices(float cropXMax, float cropYMax, float cropXMin, float cropYMin)
        {
            ...
            do
            {
                ...
                else if (polygonEntrance.HasValue)
                {
                    ...
                    polygon = new Vertices(CreateSimplePolygon(polygonEntrance.Value, new Vector2(polygonEntrance.Value.x + cropXMin + 1, polygonEntrance.Value.y)));
                }
                else
                {
                    break;
                ...
                if (polygon.Count > 2)
                 {
                    ...
                    Vertices croppedPolygon = new Vertices();

                    foreach (var x in polygon)
                    {
                        ...
                        if (x.x > cropXMin && x.x < cropXMax)
                        {
                            croppedPolygon.Add(new Vector2(croppedX, croppedY));
                        }
                    }
                    if (croppedPolygon.Count > 3)
                    {
                        detectedPolygons.Add(croppedPolygon);
                    }
