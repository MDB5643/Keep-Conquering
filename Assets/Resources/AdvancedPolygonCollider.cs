    ...
    public class AdvancedPolygonCollider : MonoBehaviour
    {
        ...
        public void PopulateCollider(PolygonCollider2D collider, ref PolygonParameters p)
        {
            try
            {
                ...
                int width;
                int height;
                if ((int)p.Rect.width < cropXMin)
                {
                    width = cropXMin;
                }
                else if ((int)p.Rect.width > cropXMax)
                {
                    width = cropXMax;
                }
                else
                ...
                if ((int)p.Rect.height < cropYMin)
                {
                    height = cropYMin;
                }
                else if ((int)p.Rect.height > cropYMax)
                {
                    height = cropYMax;
                }
                else
                ...
                int x = (int)p.Rect.x;
                int y = (int)p.Rect.y;
                UnityEngine.Color[] pixels = p.Texture.GetPixels(x, y, width, height, 0); //w68 h65
                if (width > 0 && height > 0)
                {
                    List<Vertices> verts = geometryDetector.DetectVertices(pixels, width, p.AlphaTolerance, cropXMax, cropYMax, cropXMin, cropYMin);
                    int pathIndex = 0;
                    List<Vector2[]> list = new List<Vector2[]>();
